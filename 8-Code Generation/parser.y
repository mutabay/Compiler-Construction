...
subProgList: subProgList subProgHead varDec compStmt SCOLON	{ $$ = (N_PROG *) malloc(sizeof(N_PROG));
								  $$->symtab = $2;
								  $2->next = $3;
								  $$->stmt = $4;
								  $$->next = $1;
...
subProgCall: IDENT				{ $$ = (N_CALL *) malloc(sizeof(N_CALL *));
						  $$->id = $1;
						  $$->par_list = 0;
						}
		| IDENT params			{ $$ = (N_CALL *) malloc(sizeof(N_CALL *));
						  $$->id = $1;
						  $$->par_list = $2;
						}
		;
...
assStmt: IDENT ASSIGN expr			{ $$ = (N_ASSIGN *) malloc(sizeof(N_ASSIGN));
						  $$->var_ref = (N_VAR_REF *) malloc(sizeof(N_VAR_REF));
						  $$->var_ref->id = $1;
						  $$->var_ref->index = 0;
						  $$->rhs_expr = $3;
						}
		| IDENT index ASSIGN expr	{ $$ = (N_ASSIGN *) malloc(sizeof(N_ASSIGN));
						  $$->var_ref = (N_VAR_REF *) malloc(sizeof(N_VAR_REF));
						  $$->var_ref->id = $1;
						  $$->var_ref->index = $2;
						  $$->rhs_expr = $4;
...
simpleExpr: simpleExpr addOp term	{ $$ = (N_EXPR *) malloc(sizeof(N_EXPR));
					  $$->typ = OP;
					  $$->desc.operation.op = $2;
					  $$->desc.operation.expr = $1;
					  $$->desc.operation.expr->next = $3;
...
factor: NUM				{ $$ = (N_EXPR *) malloc(sizeof(N_EXPR));
		...
		| IDENT			{ $$ = (N_EXPR *) malloc(sizeof(N_EXPR));
					  $$->typ = VAR_REF;
					  $$->desc.var_ref = (N_VAR_REF *) malloc(sizeof(N_VAR_REF));
					  $$->desc.var_ref->id = $1;
					  $$->desc.var_ref->index = 0;
					}
		| IDENT	index		{ $$ = (N_EXPR *) malloc(sizeof(N_EXPR));
					  $$->typ = VAR_REF;
					  $$->desc.var_ref = (N_VAR_REF *) malloc(sizeof(N_VAR_REF));
					  $$->desc.var_ref->id = $1;
                                          $$->desc.var_ref->index = $2;
					}
		| IDENT params		{ $$ = (N_EXPR *) malloc(sizeof(N_EXPR));
					  $$->typ = FUNC_CALL;
